...
def build_message(generation_config, tokenizer,message=[]):
    his_message = copy.deepcopy(message)
    query= ""
    history= []
    system=""
    ...
    if message:
        ...
        if message[0].get("role","") == "system":
            system = message[0].get("content")
            # 去掉system
            his_message = his_message[1:]
        ...
        # 初始化 history
        if len(his_message) > 0 and len(his_message) % 2 == 0:
            for i in range(0,len(his_message),2):
                if his_message[i].get("role") == "user" and his_message[i+1].get("role") == "assistant":
                    user = his_message[i].get("content")
                    assistant = his_message[i+1].get("content")
                    history.append((user,assistant))

    return _build_prompt(generation_config,tokenizer,query,history=history,system=system)